...
export async function GET(request: Request) {
  ...
  const cardBackId = searchParams.get('cardBackId');
  if (!cardBackId) return NextResponse.json({ error: 'cardBackId requis' }, { status: 400 });

  const supabaseAdmin = getAdminClient();
  const { data, error } = await supabaseAdmin
    .from('user_card_back_prints')
    .select('id, card_back_id, print_number, max_prints, owner_id, is_tradeable, assigned_at, created_at')
    .eq('card_back_id', Number(cardBackId))
    .order('print_number');
