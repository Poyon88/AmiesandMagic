...
// GET /api/card-prints?cardId=X — list prints for a card
export async function GET(request: Request) {
  ...
  const { data, error } = await supabaseAdmin
    .from('card_prints')
    .select('id, card_id, print_number, max_prints, owner_id, is_tradeable, assigned_at, created_at')
    .eq('card_id', Number(cardId))
    .order('print_number');

  if (error) return NextResponse.json({ error: error.message }, { status: 500 });

  // Fetch owner usernames for assigned prints
...
// PATCH /api/card-prints — assign/unassign a print, toggle tradeable
export async function PATCH(request: Request) {
  ...
  try {
    ...
    console.error('[card-prints] error:', err);
