                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                    {visibleKws.map((kw) => {
                      ...
                      const desc = kwDef?.desc
                        ...
                        : null;
                      // Conferred keyword scope: green = all allies, white = single target.
                      const grantScope = card.keyword_instances?.find((k) => k.id === kw)?.grantScope ?? "target";
                      const grantTint = grantScope === "all_allies" ? "green" : "white";
                      ...
                      return (
                        <div key={kw} style={{ display: "flex", alignItems: "flex-start", gap: 6 }}>
                          <span style={{ flexShrink: 0 }}>
                            <KeywordIcon symbol={KEYWORD_SYMBOLS[kw] || "✦"} size={16} keyword={kw} tint={grantTint} />
                          </span>
                          <div>
