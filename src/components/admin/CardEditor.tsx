    ...
    try {
      ...
      const keywordInstances = activeKeywords
        .map((id): KeywordInstance | null => {
          ...
          // Pureté +X/+Y : même traitement que ses jumelles (sans garde
          // !isSpellCard — un sort peut la conférer, son `y` doit survivre).
          if (id === "purete") {
      ...
      <div style={{ flex: 1, minHeight: 0, display: "flex", overflow: "hidden" }}>
        ...
        {selectedCard && (
          <div style={{ width: 320, borderLeft: "1px solid #e0e0e0", background: "#fff", overflow: "auto", WebkitOverflowScrolling: "touch", padding: "16px 14px", flexShrink: 0 }}>
                        ...
                        <div key={kw} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          <div style={{ display: "inline-flex", gap: 3 }}>
                            {(["play", "death", "tap", "return", "end_of_turn", "start_of_turn", "attack", "draw", "low_hp", "wound"] as const).map(mode => {
                              ...
                              const color = mode === "play" ? "#333" : mode === "death" ? "#a83232" : mode === "tap" ? "#F68D09" : mode === "return" ? "#3a7dd4" : mode === "attack" ? "#E735F6" : mode === "draw" ? "#17b6c4" : mode === "low_hp" ? "#8B5CF6" : mode === "wound" ? "#B6E62E" : mode === "start_of_turn" ? "#A9B8CC" : "#2faa3f";
                              const glyph = mode === "play" ? "⚡" : mode === "death" ? "💀" : mode === "tap" ? "⟲" : mode === "return" ? "↩" : mode === "attack" ? "⚔" : mode === "draw" ? "📥" : mode === "low_hp" ? "🩸" : mode === "wound" ? "🩹" : mode === "start_of_turn" ? "🌅" : "⌛";
                              return (
                                <button
                                  key={mode}
                                  disabled={!allowed}
                                  ...
                                  title={mode === "play" ? "À l'arrivée en jeu (défaut)" : mode === "death" ? "À la mort (deathrattle)" : mode === "tap" ? "Activable (tap / engagement)" : mode === "return" ? "Au retour en main" : mode === "attack" ? "À l'attaque" : mode === "draw" ? "Quand cette carte est piochée" : mode === "low_hp" ? "Quand votre héros passe sous 15 PV (une seule fois)" : mode === "wound" ? "Blessure : quand elle subit des dégâts sans mourir (une fois par source et par tour)" : mode === "start_of_turn" ? "Au début du tour (après la pioche)" : "À la fin du tour"}
                                  style={{
            ...
            {/* Pureté — jumelle des deux blocs ci-dessus, condition inverse :
                cimetière VIDE. Même piège, mêmes quatre morceaux (état,
                chargement, sauvegarde, champ) — cf. xy-editor-coverage.test.ts. */}
            {((editFields.keywords as string[]) || []).includes("purete") && (
              <div style={{ marginBottom: 8, padding: "8px 10px", borderRadius: 6, border: "1px solid #d9cfe8", background: "#f8f4ff" }}>
                <div style={{ ...S.label, color: "#6b4fa0", marginBottom: 6 }}>🕊️ PURETÉ (cimetière vide)</div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: 9, color: "#6b4fa0" }}>+ATK (X)</span>
                  <input
                    type="number" min={0} max={20} value={keywordXValues["purete"] ?? 1}
                    onChange={e => setKeywordXValues(prev => ({ ...prev, ["purete"]: Math.max(0, Math.min(20, parseInt(e.target.value) || 0)) }))}
                    style={{ width: 48, padding: "2px 6px", borderRadius: 4, border: "1px solid #d9cfe8", fontSize: 11, textAlign: "center" }}
                  />
                  <span style={{ fontSize: 9, color: "#6b4fa0" }}>+PV (Y)</span>
                  <input
                    type="number" min={0} max={20} value={purY}
                    onChange={e => setPurY(Math.max(0, parseInt(e.target.value) || 0))}
                    style={{ width: 48, padding: "2px 6px", borderRadius: 4, border: "1px solid #d9cfe8", fontSize: 11, textAlign: "center" }}
                  />
                </div>
              </div>
